...
class EMEDEvaluator(DatasetEvaluator):
    """
    Evaluate instance detection outputs using COCO's metrics 
    """
    ...
    def __init__(self, dataset_name, cfg, distributed, output_dir=None):
        ...
        if not hasattr(self._metadata, "json_file"):
            ...
            convert_to_coco_json(dataset_name, output_dir + "/dataset_coco_convert.json")
            self._metadata.json_file = output_dir + "/dataset_coco_convert.json"
        self._is_splits = "all" in dataset_name or "base" in dataset_name \
            or "novel" in dataset_name
        name2id = self._metadata.name2id
