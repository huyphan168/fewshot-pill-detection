...
from .meta_emed import register_meta_emed
...
def register_all_coco(root="/mnt/disk1/huyvinuni/datasets/"):
    ...
    for prefix in ["all", "novel"]:
        for shot in [1, 2, 3, 5, 10, 30]:
            for seed in range(0,10): #In the original paper, seed also set to be 0
...
def register_all_emed(root="/home/vishc1/datasets"):
    ...
    METASPLITS = [
        ...
        (
            "vaipe_train_base_geo",
            "vaipe/train/images",
            "vaipe/annotations/instances_train_geo.json",
        ),
    ...
    for name, imgdir, jsondir in METASPLITS:
        register_meta_emed(
            name,
            _get_builtin_metadata("emed"),
            os.path.join(root, imgdir),
            os.path.join(root, jsondir),
        )
